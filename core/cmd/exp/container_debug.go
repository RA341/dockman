...
func execDebugContainer(containerID string, image string, entryCmd string) (client.HijackedResponse, string) {
	...
	create, err := clu.ContainerCreate(
		...
		client.ContainerCreateOptions{
			Name: fmt.Sprintf("debug-%s", runId),
	...
	_, err = clu.ContainerStart(ctx, create.ID, client.ContainerStartOptions{})
	if err != nil {
		log.Fatal(err)
	}
	...
	resp, err := clu.ContainerAttach(ctx, create.ID, client.ContainerAttachOptions{
		Stream: true,
		Stdin:  true,
		Stdout: true,
		Stderr: true,
	...
	return resp.HijackedResponse, create.ID
}
